...
// ============ وظائف إرسال إشعارات Push (Firebase) ============
...
window.sendPushNotification = async function() {
    ...
    if (!title || !body) {
        return Swal.fire({ title: 'خطأ', text: 'يرجى إدخال العنوان ونص الرسالة', icon: 'warning' });
    }
    ...
    // إعداد بيانات الإرسال لـ Firebase
    const notificationData = {
        "to": "/topics/all_offices",
        "notification": {
            "title": title,
            "body": body,
            "sound": "default",
    ...
    // مفتاح الخادم (Server Key) - يجب استبداله بالمفتاح الحقيقي من إعدادات Firebase
    const SERVER_KEY = 'AAAA7W_pX6M:APA91bH5V7X8Y9Z0...'; // سيقوم المستخدم باستبداله
    ...
    try {
        ...
        if (response.ok) {
            Swal.fire({ title: 'تم الإرسال!', text: 'تم إرسال التنبيه لجميع المكاتب بنجاح', icon: 'success' });
            closeNotificationModal();
            addAuditEntry('إشعار', `إرسال تنبيه عام: ${title}`, body);
        } else {
            throw new Error('فشل الإرسال من السيرفر');
        }
    } catch (error) {
        console.error("Error sending notification:", error);
        Swal.fire({ 
            title: 'فشل الإرسال', 
            text: 'تأكد من إعداد مفتاح الخادم (Server Key) بشكل صحيح في ملف users_management.js', 
            icon: 'error' 
        });
    }
};

// إظهار زر الإشعارات للمدير فقط عند تسجيل الدخول
